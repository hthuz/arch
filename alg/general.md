...
但是数字证书也可能被篡改，因此需要digital signature 数字签名 = privateCA(hash(plain text))
...
->浏览器用公钥对签名进行解密，获取hash(plain text)

->对hash对plain text进行, 获取hash(plain text)

验证两个是否相等，不相等，说明签名或明文被篡改
...
- 对锁的竞争会很激烈，cache频繁失效，让CPU缓存命中率下降

- 内存碎片化，生成很多大小不同的内存块，内存空间被分为很多小块，无法满足大内存需求
- 容易有一致性问题，内存泄露问题，比如忘记释放或者重复释放

解决方案：

对于内存泄露：进行实时的监控和检测，定期dump离线分析，让不同业务使用独立的内存池

高并发时的new & delete策略

- 每个线程维护独立的内存池，完全避免锁竞争

- 不立即释放内存，而是积累到一定数量后批量释放
- 使用带引用计数的智能指针，自动管理生命周期
- 对于频繁创建销毁的同类型对象，采用对象复用



## 内存池/对象池

内存池：向系统申请一大块内存，然后按需从这块内存里切分小块给程序使用；释放时回收到池中，而不是立刻还给操作系统

=> 减少malloc/new/free/delete 频率

适用高频小块的内存分配，同时可以做到线程本地缓存，减少锁的竞争



对象池：预先创建一批对象实例，需要是从池中借一个，不用时把对象归还
...
## 传统odps问题， delta table的特点

delta table：离线 近实时一体化

传统需求：对时效性要求低的离线数据进行全量批处理

新需求：近实时和增量处理链路



如果纯离线批处理，每隔一段时间进行全量写读，那么其时效性差，出错时回溯链路很长，且数据合并会造成冗余的存储计算成本
...
lambda： 流处理 + 批处理，流处理获取最新的数据，但一致性是一个问题
